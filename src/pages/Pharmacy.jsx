  ...
  return (
    <Layout title="Pharmacy">
      <main className="relative px-4 sm:px-6 py-8">
        ...
        {/* TITLE */}
        <p className="text-sm text-gray-500 mb-4">Manage medicines in stock</p>

        {/* TABLE */}
        <div className="bg-white rounded-xl shadow overflow-x-auto">

          {medicines.length === 0 ? (
            <p className="p-10 text-center text-gray-500">No medicines yet</p>
          ) : (
            <table className="min-w-[600px] sm:min-w-full text-sm">
              ...
              <tbody>
                {medicines.map(med => (
                  <tr key={med._id} className="border-b hover:bg-gray-50 transition">
                    <td className="px-4 py-2">
                      <span className="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-700">{med.category}</span>
                    </td>
                    <td className="px-4 py-2 font-medium text-gray-800">{med.name}</td>
                    <td className="px-4 py-2">{med.price ? `$${med.price}` : "-"}</td>
                    <td className="px-4 py-2">{med.quantity}</td>
                    <td className="px-4 py-2">{med.expDate ? formatDate(med.expDate) : "-"}</td>
                    <td className="px-4 py-2 text-gray-500">{formatDate(med.createdAt)}</td>
        ...
        <button
          ...
          className="fixed bottom-6 sm:bottom-8 right-6 sm:right-8 w-14 h-14 bg-teal-600 hover:bg-teal-700 text-white text-2xl rounded-full shadow-lg flex items-center justify-center transition"
        >
          +
        </button>
        ...
        {showForm && (
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
            <div className="bg-white w-full max-w-md sm:max-w-lg rounded-xl shadow-xl p-6">
              ...
              <h2 className="text-lg font-semibold mb-6">Add Medicine</h2>
